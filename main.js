...
;// CONCATENATED MODULE: ./src/js/components/task-board/task-board.js
...
class TaskBoard {
  ...
  bindToDOM() {
    ...
    this.taskColumnElements = this.element.querySelectorAll(TaskBoard.taskColumnSelector);
    this.taskListElements = this.element.querySelectorAll(TaskBoard.taskListSelector);
    this.taskAddButtonElement = this.element.querySelector(TaskBoard.taskAddButtonSelector);
  ...
  showFormAddTask(e) {
    ...
    if (currentElement.classList.contains("task-column__add-task-btn")) {
      currentElement.closest(TaskBoard.taskColumnSelector).insertAdjacentHTML("beforeEnd", this.formAddTask);
  ...
  removeFormAddTask(e) {
    ...
    if (currentElement.classList.contains("form-add-task__cancel-btn")) {
      ...
      form.remove();
      column.insertAdjacentHTML("beforeEnd", '<button class="task-column__add-task-btn">Add another card</button>');
    }
  }
  ...
  removeTask(e) {
    ...
    if (currentElement.classList.contains("task__delete-btn")) {
      this.taskBuffer = this.taskBuffer.filter(task => +currentTask.id !== task.id);
      currentTask.remove();
      this.saveTasks();
      this.reloadTaskList();
    }
  ...
  reloadTaskList() {
    ...
    this.taskListElements.forEach(list => {
      ...
      this.taskBuffer.forEach(task => {
        const taskElement = this.renderTask(task.taskListId, task.id, task.text);
  ...
  onDradTask(e) {
    ...
    if (belowElement.classList.contains("task")) {
      if (document.querySelector(".dragged-shadow")) document.querySelector(".dragged-shadow").remove();
      if (this.element.querySelector(".dragged-shadow")) return;
      belowElement.insertAdjacentElement("beforeBegin", shadowElement);
    }
    if (belowElement.classList.contains("task-column__add-task-btn") || belowElement.classList.contains("form-add-task__text")) {
      if (document.querySelector(".dragged-shadow")) document.querySelector(".dragged-shadow").remove();
      if (this.element.querySelector(".dragged-shadow")) return;
  ...
  onDropTask(e) {
    ...
    this.taskBuffer.forEach(task => {
      ...
      if (task.id === +this.dragElement.id && task.taskListId !== currentTaskList.id) {
...
;// CONCATENATED MODULE: ./src/js/components/form-add-task/form-add-task.js
